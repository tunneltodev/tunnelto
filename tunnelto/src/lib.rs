...
/// main entrypoint for the tunnelto client
pub async fn run(
    config: Config,
    on_connect_tx: Option<UnboundedSender<ConnectedHost>>,
    kill_rx: oneshot::Receiver<()>,
) {
    let wormhole_fut = async move {
        run_wormhole(config, on_connect_tx).await;
    };

    tokio::select! {
        res = wormhole_fut => res,
        _ = kill_rx => {
...
async fn run_wormhole(mut config: Config, on_connect_tx: Option<UnboundedSender<ConnectedHost>>) {
    let introspect_dash_addr = introspect::start_introspect_web_dashboard(config.clone());

    loop {
        let (restart_tx, mut restart_rx) = unbounded();
        let wormhole = run_wormhole_inner(
            config.clone(),
            introspect_dash_addr.clone(),
            restart_tx,
            on_connect_tx.clone(),
        );
...
async fn run_wormhole_inner(
    ...
    if let Some(mut tx) = on_connect_tx {
        let _ = tx
            .send(ConnectedHost {
...
async fn process_control_flow_message(
    ...
    match &control_packet {
        ...
        ControlPacket::Terminate { reason } => {
            info!("got terminate message: {}", reason);
            eprintln!(
                "\nServer terminated connection: {}",
                reason.red().underline()
            );
            std::process::exit(1);
        }
    };
